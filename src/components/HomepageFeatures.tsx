...
function BlankLink(props) {
  if (!props.externalLink) {
    return (
      <a href={`${props.baseURL}${props.href}`}>
...
function Feature({ title, image, href, description, externalLink, baseURL }: FeatureItem & { baseURL: string }) {
  return (
    <div className={clsx('col col--3')}>
      <BlankLink title={title} image={image} href={href} description={description} externalLink={externalLink} baseURL={baseURL} />
    </div>
  );
}

export default function HomepageFeatures(props: { baseURL: string }): JSX.Element {
  const { baseURL } = props;
  return (
      ...
      <div className="container">
        <div className="row">
          {FeatureList.map((props, idx) => (
            <Feature key={idx} {...props} baseURL={baseURL} />
          ))}
        </div>
      </div>
    </section>
